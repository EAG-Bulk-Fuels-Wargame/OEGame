...
public class InfoBarScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!stopSearch)
            if (Input.GetMouseButton(0) && !actionOpen && GetMouseCell() != null)
            {
                ...
                if (UnitAct.HasTeamUnit(user.GetTeam(),GetMouseCell()))
                    Words[0].text = ("<b>Name:</b> " + GetMouseCell().name + " \t <b>Building:</b> " + GetMouseCell().building + "\n <b>Terrain:</b> " + GetTerrain(GetMouseCell().Color) + " \t <b>Unit:</b> " + UnitAct.GetTeamUnit(user.GetTeam(), GetMouseCell()).GetUnitName() + " (" + UnitAct.GetTeamUnit(user.GetTeam(), GetMouseCell()).GetStacked() + ")");
                else
    ...
    public void UnitAction(HexCell h, Unit u)
    {
        ...
        ActionProcess.MakeAction(u, vector, GetOptions(h, u), GetScenarioName(h, u));
    }
    ...
    public List<string> GetOptions(HexCell h, Unit u)
    {
        ...
        {
            s.Add("Allocate Additional Fuel to Unit");
            s.Add("Build Camp");
            s.Add("Establish Farp");
            s.Add("Ground Raid");
            s.Add("Ground Reconnaissance");
            s.Add("Ground Unit Relocation");
            s.Add("Maintain Security Operations at Current Location");
            s.Add("NGO Security Detail");
            s.Add("Propaganda");
            s.Add("Public Works Project");
            s.Add("Reduced Security Operations at Location");
            s.Add("Repair Critical Energy Infrastructure");
            s.Add("SOF Raid");
