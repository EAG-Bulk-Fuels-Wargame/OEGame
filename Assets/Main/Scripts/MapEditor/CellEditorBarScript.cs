...
public class CellEditorBarScript : MonoBehaviour {
    ...
    private void Update()
    {
        ...
        if (GetMouseCell() != null && Input.GetMouseButton(0))
        {
            ...
            fuelRegion = currentHex.fuelRegion;
            Fields[0].text = name = currentHex.name;
            Debug.Log(currentHex.name);
            Debug.Log(Words.Length);
            Debug.Log(Words[2].text);
            Debug.Log(fuelRegion.name);
            Words[2].text = fuelRegion.name + " Fuel Amount:";
            Fields[1].text = fuelRegion.fuelQty + "";
    ...
    {
        int qty;
        int.TryParse(Fields[1].text, out qty);
        Debug.Log("Text read: " + qty);
        if (int.TryParse(Fields[1].text, out qty))
            fuelRegion.fuelQty = qty;
        else
            Fields[1].text = "ERROR: Please input an integer";
    }
